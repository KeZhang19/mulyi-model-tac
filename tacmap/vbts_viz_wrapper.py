...
class VBTSVizWrapper:
    def __init__(self, env, show=True, env_idx=[0]):
        ...
        self.sensor_points = []
        for vbts_cfg in self.cfg.vbts_sensor:
            scale = float(getattr(vbts_cfg, "correction_scale", 1.0))
            self.sensor_points.append(np.load(vbts_cfg.points_npy) * scale)
    ...
    def _get_2d_point(self, point_3d, sensor_id):
        if (
            point_3d is None
            or sensor_id >= len(self.sensor_points)
            or np.linalg.norm(point_3d) <= 1e-3
        ):
            return None

        points = self.sensor_points[sensor_id].reshape(-1, 3)
    ...
    def _update_panel(self, observations):
        ...
        for env_id in self._env_idx:
            for sensor_id in self._sensor_idx:
                ...
                if vbts.ndim == 4 and vbts.shape[2] == self._H and vbts.shape[3] == self._W:
                    # sensor index check
