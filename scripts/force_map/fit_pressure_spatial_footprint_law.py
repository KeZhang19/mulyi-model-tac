...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit a spatial taxel footprint law against a dense model reference."
    )
    parser.add_argument("trace", type=Path, help="Pressure trace .npz containing support/depth fields.")
    ...
    parser.add_argument("--transition-depths-m", type=_float_list, default=None)
    parser.add_argument("--early-ops", type=_str_list, default=None)
    parser.add_argument("--late-ops", type=_str_list, default=None)
    parser.add_argument("--early-iterations", type=_int_list, default=None)
    parser.add_argument("--late-iterations", type=_int_list, default=None)
...
def main() -> int:
    args = parse_args()
    cfg = PressureSpatialFootprintFitConfig(
    ...
    result = fit_pressure_spatial_footprint_to_reference(
        ...
        transition_depths_m=args.transition_depths_m,
        early_ops=args.early_ops,
        late_ops=args.late_ops,
        early_iterations=args.early_iterations,
        late_iterations=args.late_iterations,
    )
    if args.fit_trace_out is not None:
        best = result["best"]
        fit_path = write_spatial_footprint_fit_trace(
            args.trace,
            args.fit_trace_out,
            ...
            transition_depth_m=float(best["transition_depth_m"]),
            early_op=str(best["early_op"]),
            late_op=str(best["late_op"]),
            early_iterations=int(best["early_iterations"]),
            late_iterations=int(best["late_iterations"]),
        ...
        result["fit_verify_command"] = pressure_fit_verify_command(
            fit_path,
            pressure_key="pressure_norm_spatial_footprint_fit",
            raw_pressure_key="pressure_raw_spatial_footprint_fit_n",
            penetration_key="penetration_spatial_footprint_fit_m",
            reference_key=str(args.reference_key),
            args=args,
        )
