...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit pressure active-mask and depth calibration separately against a dense model reference."
    )
    parser.add_argument("trace", type=Path, help="Pressure trace .npz containing a penetration map and reference map.")
...
def main() -> int:
    args = parse_args()
    cfg = PressureMaskDepthFitConfig(
    ...
    if args.fit_trace_out is not None:
        best = result["best"]
        fit_path = write_mask_depth_fit_trace(
            args.trace,
            args.fit_trace_out,
        ...
        result["fit_verify_command"] = pressure_fit_verify_command(
            fit_path,
            pressure_key="pressure_norm_mask_depth_fit",
            raw_pressure_key="pressure_raw_mask_depth_fit_n",
            penetration_key="penetration_mask_depth_fit_m",
            reference_key=str(args.reference_key),
            args=args,
        )
