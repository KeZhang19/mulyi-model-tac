...
The encoders are loaded from checkpoints and frozen by default.  A paired
dataset supplies one simulation sample and one real sample for the same
``(episode_id, episode_step)`` key; all mismatched keys in a minibatch become
in-batch negatives for the symmetric InfoNCE objective.
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train frozen-encoder CTTP-style tactile latent alignment heads."
    )
    ...
    *,
    ...
    with context:
        for batch_index, batch in enumerate(loader):
            ...
            positive_mask = build_positive_mask(pair_ids, pair_ids)
            outputs = aligner(_inputs(sim_batch), _inputs(real_batch))
            loss = aligner.alignment_loss(outputs, positive_mask)
            if training:
            ...
            total_recall += _retrieval_recall(outputs, positive_mask)
            batches += 1
    if batches == 0:
...
def _save_checkpoint(
    ...
    *,
    ...
    torch.save(
        {
            "schema_version": 1,
            "training_objective": "cttp_style_frozen_encoder_latent_alignment_v1",
            "epoch": int(epoch),
