...
"""Revo3 bulb manipulation with the threaded HiveBoard lamp on the table."""
...
from ...mdp.rotate_bulb_tactile import RotateBulbPretrainedTactile, initialize_rotate_bulb_tactile_contract
...
_ROBOT_BASE_BOTTOM_OFFSET = 0.006  # Base_R USD mesh minimum local Z is -6 mm.
# Offline USD kinematics solution: compensate the 186 mm base lift while
# preserving the original palm/wrist pose. Fingers and the root yaw are unchanged.
_TABLETOP_ARM_JOINT_POS = {
    "Joint1_R": -0.506634,
    "Joint2_R": -0.649688,
    "Joint3_R": 1.570792,
    "Joint4_R": -0.171813,
...
def reset_lamp_on_table(
    ...
    # Table poses already contain each environment's origin and this reset's
    # table randomization. Sample independently in that table's local XY frame.
...
def configure_rotate_bulb_scene(cfg):
    ...
    # Move this task's table beneath the existing robot/socket XY positions.
    # The fixed base mounts on the tabletop; its origin is above its mesh bottom.
    cfg.scene.table.init_state.pos = (0.55, 0.0, 0.38)
    table_top = cfg.scene.table.init_state.pos[2] + cfg.scene.table.spawn.size[2] / 2
    cfg.scene.robot.init_state.pos = (1.0, 0.0, table_top + _ROBOT_BASE_BOTTOM_OFFSET)
    cfg.scene.robot.init_state.joint_pos.update(_TABLETOP_ARM_JOINT_POS)
    ...
    cfg.events.reset_object = EventTerm(
        ...
        mode="reset",
        params={
            "x_range": _LAMP_X_RANGE,
            "y_range": _LAMP_Y_RANGE,
    ...
    cfg.terminations.object_out_of_bound.params["in_bound_range"] = {
        "x": (0.1, 0.75), "y": (-0.35, 0.55), "z": (0.65, 1.3),
    }
    cfg.terminations.bulb_dropped = DoneTerm(func=rotate_bulb_dropped)
    cfg.scene.lamp.init_state.pos = (
        cfg.scene.table.init_state.pos[0] + sum(_LAMP_X_RANGE) / 2,
        cfg.scene.table.init_state.pos[1] + sum(_LAMP_Y_RANGE) / 2,
        cfg.scene.table.init_state.pos[2] + cfg.scene.table.spawn.size[2] / 2 + _SUPPORT_BOTTOM_OFFSET,
    )
...
class RotateBulbTransportProgress(ManagerTermBase):
    """Reward new best goal approach only after establishing a released baseline."""

    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.best = torch.zeros(env.num_envs, device=env.device)
    ...
    def __call__(self, env, std: float, command_name: str, asset_cfg: SceneEntityCfg,
        ...
        released = env._bulb_released
        first = released & ~self.initialized
        self.best = torch.where(first, potential, self.best)
        delta = (potential - self.best).clamp_min(0.0)
        # Consume unheld progress too, so throwing then regrasping cannot pay it later.
        self.best = torch.where(released, torch.maximum(self.best, potential), self.best)
        self.initialized |= released
        contact = rotate_bulb_contact_scores(env).min(dim=-1).values
        contact = torch.nan_to_num(contact, nan=0.0, posinf=0.0, neginf=0.0)
        return delta * contact * (released & ~rotate_bulb_failed(env)).float() / env.step_dt
...
class RotateBulbStableSuccess(ManagerTermBase):
    """Require unscrewing, clearance, grasp and a settled hold at the goal."""
    ...
    def __call__(
        ...
        # rot_std is retained for compatibility with the lift base config;
        # this task asks for a position, not a random final orientation.
        ...
        # Require clearance from the socket in addition to the release flag.
        clear = bulb.data.root_pos_w[:, 2] - env.scene["support"].data.root_pos_w[:, 2] > min_clearance
        settled = (torch.linalg.vector_norm(bulb.data.root_lin_vel_w, dim=-1) < max_linear_speed)
        settled &= torch.linalg.vector_norm(bulb.data.root_ang_vel_w, dim=-1) < max_angular_speed
        valid = close & clear & settled & rotate_bulb_screw_complete(env) & rotate_bulb_hand_contacts(env, 1.0)
        valid &= env._bulb_released
...
class DexsuiteRevo3RotateBulbRewardCfg:
    """Bounded phase budgets: reach 1, grasp 1, turns 6, hold 4, carry 4, success 10.

    Event/progress terms divide by step_dt; reward-rate terms integrate seconds.
    Turn budget is 2 points per turn (6 for the default three-turn start).
    """

    action_l2 = RewTerm(func=mdp.action_l2_clamped, weight=-0.001)

    action_rate_l2 = RewTerm(func=mdp.action_rate_l2_clamped, weight=-0.001)

    # Keep the descriptor for the shared Revo3 mixin's fingertip selection.
    # Static proximity and duplicate contact occupancy no longer earn rewards.
    ...
    unscrew_progress = RewTerm(
        func=RotateBulbUnscrewProgress, weight=2.0,
